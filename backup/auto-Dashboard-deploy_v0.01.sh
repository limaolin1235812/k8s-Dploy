...
# auto Dashboard UI deploy
# by author limaolin
##############################################

ssh-keyscan
for ip in `cat /etc/hosts|grep node|awk '{print $3}'`;
do
	ssh-copy-id $ip;
done
#K8s Dashboard UI实战
...
#              value: zh
wget -c  https://raw.githubusercontent.com/kubernetes/dashboard/v2.0.0-rc5/aio/deploy/recommended.yaml >recommended.yaml
cp -r recommended.yaml src/
cat >>src/recommended.yaml<<-EOF
...
echo 通过浏览器访问Dashboard WEB，https://$UI:$ECU/ ，输入Token登录即可
